...
- **For API Consumers**: A self-service portal to discover APIs, request access, and manage API keys
- **For API Owners**: Tools to publish API products, approve or reject access requests, and configure tiered access plans
- **For Platform Engineers**: Complete oversight with RBAC-controlled permissions, bulk operations, and integration with existing Kuadrant policies
...
The portal implements a four-persona permission model:

**API Consumer**
- Browse all API products
- Request API keys for any published API
- View and manage only their own API keys
- No administrative capabilities

**API Owner**
- Create and publish API products they own
- Approve/reject requests for their APIs
- View rate limit policies (read-only)
- Cannot access other owners' products

**API Admin**
- Full visibility and control over all API products
- Approve/reject any API key request
- Create, update, and delete any API product

**Platform Engineer**
- Manage PlanPolicies and rate limits on the cluster
- Full administrative access via RBAC
...
The Kuadrant Developer Portal consists of two main plugins:

### Frontend Plugin (`@kuadrant/kuadrant-backstage-plugin-frontend`)
- Main Kuadrant page with API products grid and approval queue
- API key management tabs integrated into API entity pages
- API product info cards showing tiers and documentation
- Permission-gated UI components with RBAC enforcement
- Filter panels, bulk operations, and inline editing

### Backend Plugin (`@kuadrant/kuadrant-backstage-plugin-backend`)
- HTTP API for APIProduct and APIKey CRUD operations
- Kubernetes client integration using `@kubernetes/client-node`
- APIProduct entity provider for automatic catalog sync
- RBAC permission checks with ownership verification
- Support for in-cluster authentication and explicit cluster configuration

### Kubernetes Custom Resources
- **APIProduct**: Defines API products with metadata, documentation, plan tiers, and publish status
- **APIKey**: Tracks access requests with status (Pending/Approved/Rejected), plan tier, and use case
- **PlanPolicy**: Configures rate limit tiers with predicates and limits (managed on cluster by platform engineers)
